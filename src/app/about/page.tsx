...
import { FaBriefcase, FaHtml5, FaNodeJs, FaReact } from "react-icons/fa";
    ...
    <>
      <div className="px-49">
        <div className="text-center pt-14 pb-24">
          ...
          <h1 className="font-extrabold text-5xl">About <span className="text-amber-400">Me</span></h1>
        </div>
          ...
          <div className="flex">
            ...
            <div className="pl-10">
              <div>
                ...
                <div className="pt-3">
                  <button className="border-amber-400 border-2 py-2 px-9 rounded-full font-semibold text-lg hover:bg-amber-400 transition-all duration-300 ease-in-out">
                    Download CV
                  </button>
                </div>
              </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-36">
        {skills.map((skill, index) => (
          <div key={index} className="flex flex-col items-center">
            ...
            <p className="text-white mt-2 font-semibold text-md">{skill.name}</p>
          </div>
        ))}
      </div>
    </div>
        ...
        <div className="space-y-8">
          {education.map((item, index) => (
            <div key={index} className="flex items-start gap-4 relative">
              ...
              {/* Content */}
              <div className="flex-1 border-l border-gray-700 pl-6">
                <span className="text-gray-400 font-[600] px-2 py-0.5 bg-[#252525] rounded-full text-xs">{item.year}</span>
                <h3 className="font-bold text-lg mt-1">
                  {item.title} <span className="text-gray-400">— {item.school}</span>
                </h3>
                <p className="mt-1 text-gray-300">{item.description}</p>
